...
class PromiseToggle<T1, T2> {
  constructor(
    initState: { type: 'open'; value: T1 } | { type: 'close'; value: T2 },
  ) {
...
export class Server_external extends HttpServer {
  ...
  private ipcPo = new PromiseToggle<$ReadableStreamIpc, void>({
    type: 'close',
    value: undefined,
  })
    ...
    if (pathname.startsWith(`/${this.token}`)) {
      if (isWebSocket(event.method, event.headers)) {
        ...
        streamIpc.onFetch(async (event) => {
          ...
          await mapHelper.getOrPut(
            this.externalWaitters,
            mmid,
            async (_key) => {
              let ipc: $Ipc
              try {
          ...
          if (ipc && this.needActivity) {
            // 激活对面窗口
            ipc.postMessage(
              IpcEvent.fromText(ExternalState.ACTIVITY, ExternalState.ACTIVITY),
            )
          }
