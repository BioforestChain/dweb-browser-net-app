...
export const doBuidCore = async (config: {
  ...
  await dnt.build({
    ...
    package: {
      ...
      name: name,
      version: version,
      description: `WebApp-network In DwebBrowser`,
      license: 'MIT',
      repository: {
        type: 'git',
        url: 'git+https://github.com/BioforestChain/dweb-browser-net-app.git',
      },
      bugs: {
        url: 'https://github.com/BioforestChain/dweb-browser-net-app/issues',
      },
      keywords: ['network', 'dweb'],
  ...
  if (config.watch && config.watchDir) {
    const watcher = watch(path.resolve(Deno.cwd(), config.watchDir), {
      recursive: true,
    })
...
export const doBuild = async (args = Deno.args) => {
  ...
  await doBuildFromJson(import.meta.resolve(`./npm.json`), rest)
}
